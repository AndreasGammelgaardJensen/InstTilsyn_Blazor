...
namespace PDFExtractionLib.Handlers
{
    public class PDFExtractionPDFText : IPDFExtraction
    {
        ...
        public Dictionary<string, string> Extract_Categories_From_Stream(Stream fileStream)
        {
                ...
                for (int j = 0, len = words.Length; j < len; j++)
                {
                    ...
                    try
                    {
                        if (line.Contains(Tags.Tags.Section_Vurdering) || line.Contains(Tags.Tags.Section_Vurdering_v2) || line.Contains(Tags.Tags.Section_Vurdering_v3) || line.Contains(Tags.Tags.Section_Vurdering_v4))
                        {
                            ...
                            var indsats = string.Join(" ", lineArray[lineArray.Length - 2], lineArray[lineArray.Length - 1]);
                            indsats = indsats.Replace(".", "");
                            _logger.Debug(Tags.Tags.TagConverter(categories));

                            _logger.Debug(indsats);
            ...
            _logger.Debug($"Finished reading from stream\n {indsatsDict.Count} indsatser successfully collected");
